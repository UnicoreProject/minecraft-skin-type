...
export class MinecraftSkinType {
  private static hasTransparency(context: CanvasRenderingContext2D, x0: number, y0: number, w: number, h: number): boolean {
  ...
  private static isAreaBlack(context: CanvasRenderingContext2D, x0: number, y0: number, w: number, h: number): boolean {
    const imgData = context.getImageData(x0, y0, w, h);
    for (let x = 0; x < w; x++) {
      for (let y = 0; y < h; y++) {
        const offset = (x + y * w) * 4;
  ...
  private static isAreaWhite(context: CanvasRenderingContext2D, x0: number, y0: number, w: number, h: number): boolean {
    const imgData = context.getImageData(x0, y0, w, h);
    for (let x = 0; x < w; x++) {
      for (let y = 0; y < h; y++) {
        const offset = (x + y * w) * 4;
  ...
  private static computeSkinScale(width: number): number {
    return width / 64.0;
  }

  private static copyImage(context: CanvasRenderingContext2D, sX: number, sY: number, w: number, h: number, dX: number, dY: number, flipHorizontal: boolean): CanvasRenderingContext2D {
